...
ggplot(final_capstone_results, aes(x = utility_ratio, y = total_viable_acreage)) +
  # Crosshairs based on the mean of raw values
  geom_vline(xintercept = mean(final_capstone_results$utility_ratio, na.rm = TRUE), 
             linetype = "dashed", color = "grey70") +
  geom_hline(yintercept = mean(final_capstone_results$total_viable_acreage, na.rm = TRUE), 
             linetype = "dashed", color = "grey70") +
  
  # Point size is tied to the combined viability (Efficiency + Capacity)
...
# Interpretation: Middletown, Richmond, and South Kingstown emerge as Rhode Island’s primary agrivoltaic hubs due to a rare convergence of flat topography and expansive agricultural acreage. While South Kingstown serves as a capacity powerhouse offering the state's largest total volume of farmland for community-scale solar, Middletown and Richmond lead in operational efficiency. These two towns boast a higher concentration of large, contiguous, utility-scale plots that significantly lower per-megawatt installation costs for developers. This bubble plot illustrates the technical trade-offs that need to be considered in agrivoltaic site selection.  
...
# Interpretation: This Chloropleth map of the seven towns that hold the greatest agricultural land capacity as determined by total acreage. Richmond and Little Compton stand out as having the greatest suitability for implementing agrivoltaics. Therefore, these towns should be the priority focus for policy makers, solar farm developers and town planners. These two towns should be examined further to develop land ordinances that support the dual-location of solar energy and agricultural properties. Preserving the land as Farm, Forest, and Open Space for farmland preservation. 
...
# Interpretation: This stacked bar chart provides a transparent breakdown of the Multi-Criteria Decision Analysis (MCDA), illustrating how each of the three weighted factors contributes to a town's final suitability score. While Richmond leads in total weighted score, the chart reveals a high degree of balance across the top five municipalities. The narrow margin between the top contenders suggests that Rhode Island possesses a resilient "cluster" of potential agrivoltaic hubs in both South County and the East Bay, rather than one single outlier.
